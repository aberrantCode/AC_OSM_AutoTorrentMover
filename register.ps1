...
$scriptPath = "c:\osm\torrents\src\Move-Torrents.ps1"  # Update this path to the actual location of your start-monitor.ps1 script
...
function Register-ScheduledTaskTriggeredOnSchedule {
    ...
    $trigger = New-ScheduledTaskTrigger -Once -At (Get-Date) -RepetitionInterval (New-TimeSpan -Minutes $IntervalMinutes) -ErrorAction Stop

    # Register the scheduled task
    Register-ScheduledTask -Action $action -Trigger $trigger -TaskName $TaskName -Description $Description -ErrorAction Stop
...
Register-ScheduledTaskTriggeredOnSchedule -TaskName $taskName -ScriptPath $scriptPath -IntervalMinutes 5 | Out-Null
